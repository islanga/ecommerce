...
class Catalog
{
	...
	// Retrieves complete details for the specified departments
	...
	public static function GetProductsOnCatalog($pageNo, &$rHowManyPages)
	{
		...
		$sql = 'CALL catalog count products on catalog()';
		...
		// Build the parameters array
		$params = array(':short_product_description_length' => SHORT_PRODUCT_DESCRIPTION_LENGTH, ':products_per_page' => PRODUCTS_PER_PAGE,
	...
	public static function Search($searchString, $allWords, $pageNo, &$rHowManyPages)
	{
		...
		// On the first call to strtok you supply the whole search string and the list of delimiters. It returns the first word of the string 
		...
		if (count($seach_result['accepted_words']) == 0)
			return $search_result;
		...
		$params = [':search_string' => $search_string, ':all_words' => $allWords];
